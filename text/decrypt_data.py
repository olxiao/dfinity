...
def try_decrypt_data(encrypted_string, key=None, iv=None):
    """
    尝试多种方法解密数据
    """
    print("开始解密数据...")
    ...
    if data:
        ...
        try:
            ...
            print(f"JSON解析成功: {json.dumps(json_data, indent=2, ensure_ascii=False)}")
        except Exception as e:
            print(f"JSON解析失败: {e}")
    ...
    if data:
        ...
        if decompressed:
            ...
            try:
                ...
                print(f"解压后内容: {text[:500]}...")  # 只显示前500个字符
            except Exception as e:
                print(f"文本解码失败: {e}")
                print(f"十六进制表示: {binascii.hexlify(decompressed).decode()}")
        ...
        if data:
            ...
            if decrypted:
                try:
                    ...
                    print(f"AES解密成功! 解密后内容: {text[:500]}...")
                except Exception as e:
                    print(f"文本解码失败: {e}")
                    print(f"十六进制表示: {binascii.hexlify(decrypted).decode()}")
            else:
                print("AES解密失败")
    else:
...
if __name__ == "__main__":
    ...
    for i, key in enumerate(test_keys):
        for j, iv in enumerate(test_ivs):
            print(f"\n尝试密钥{i+1} + IV{j+1}:")
            try_decrypt_data(encrypted_string, key, iv)
