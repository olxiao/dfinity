...
def analyze_encrypted_string(encrypted_string):
    """分析加密字符串的特征"""
    print("="*60)
    print("加密数据特征分析")
    print("="*60)
    ...
    # 基本信息
    print(f"原始字符串长度: {len(encrypted_string)}")
    print(f"Base64解码后长度: {len(base64.b64decode(encrypted_string))}")
    ...
    try:
        ...
        print(f"  前128字符十六进制: {hex_str[:128]}")
        if len(hex_str) > 128:
            print(f"  ...(共{len(hex_str)}字符)")
            print(f"  最后128字符: {hex_str[-128:]}")

        # 检查是否是AES加密的CBC模式（应该是16字节对齐）
        block_size = 16
        if len(decoded) % block_size == 0:
            num_blocks = len(decoded) // block_size
            print(f"\n✅ 数据长度是{AES_BLOCK_SIZE}字节的倍数 ({num_blocks}个块)")
        else:
            print(f"\n❌ 数据长度不是{AES_BLOCK_SIZE}字节的倍数")

        # 检查常见的魔数或模式
        print(f"\n魔数检查:")
        print(f"  开头4字节: {binascii.hexlify(decoded[:4]).decode()}")
        print(f"  结尾4字节: {binascii.hexlify(decoded[-4:]).decode()}")
        ...
        if decoded_text:
            ...
            if '{' in decoded_text and '}' in decoded_text:
                ...
                if start != -1 and end != 0:
                    json_str = decoded_text[start:end]
                    try:
            ...
            elif '[' in decoded_text and ']' in decoded_text:
                print("  🔍 发现方括号，可能是数组")
            else:
                print("  📄 普通文本数据")
...
if __name__ == "__main__":
    ...
    result = analyze_encrypted_string(encrypted_string)
    if result:
        print(f"\n🎉 成功解密并解析为JSON!")
    else:
        print(f"\n❌ 未能成功解密")